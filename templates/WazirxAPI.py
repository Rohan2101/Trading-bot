...
def get_combined_signal(data):
    ...
    if (x == 1 and y == 1) or (y == 1 and z == 1) or (x == 1 and z == 1):
        ...
        pass
    return f'Combined Signal: {xy}, RSI Signal: {x}, MACD signal: {y}, BB Signal: {z}'
...
def get_bb_signals(data, period = 20, std = 2):
    ...
    signals['upperband'] = bb_data['upperband']
    signals['lowerband'] = bb_data['lowerband']
    signals['sell']= 0.0
    signals['buy']= 0.0
    ...
    return signals[['Datetime','price','upperband','lowerband', 'buy_sell']].iloc[-30:-1]
...
def get_Stochastic_signals(data, level=7):
    ...
    print(data[['Datetime','Close','Slow Stochastic']].iloc[-20:-1])
    return data
...
def get_Stochastic_signal(data, level=7):
    ...
    data.loc[data['Slow Stochastic'] < -level, ['buy_signal']] = 1
    data.loc[data['Slow Stochastic'] > -level, ['buy_signal']] = 0
    return f'Buy signal: {data.iloc[-1]["buy_signal"]} Sell signal: {data.iloc[-1]["sell_signal"]}'
...
def generate_combined_signals_values(data):
    #y = Get_RSI_signals(data)
    #print(y)

    z = Get_MACD_signals(data)
    print("BB Values:\n")
    print(get_bb_signals(data))
    print("Stochastic Values:\n")
    z3 = get_Stochastic_signals(data)
    print("MACD Values\n")
    return z
