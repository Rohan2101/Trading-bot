...
def signals_menu():
    ...
    print("Press 9 for generating all signal values")
    x = int(input("Press 11 for generating combined signal for the last traded price and time\n"))
    if x == 0:
        main_menu()
    elif x == 1:
        ...
        WazirxAPI.get_Stochastic_signals(y)
        signals_menu()
    elif x ==8 :
        ...
        j1 = RSI1.get_rsi(y, 14)
        print("RSI Values: ")
        print(j1[-30:-1])
        print(WazirxAPI.generate_combined_signals_values(y))
        signals_menu()
    else:
